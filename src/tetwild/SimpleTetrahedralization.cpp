...
void SimpleTetrahedralization::triangulation(std::vector<TetVertex>& tet_vertices, std::vector<std::array<int, 4>>& tets) {
    ...
    for (int i = 0; i < bsp_faces.size(); i++) {
        ...
        std::cout << MC.bsp_vertices[MC.bsp_faces[i].vertices[0]] << std::endl;
        std::cout << MC.bsp_vertices[MC.bsp_faces[i].vertices[1]] << std::endl;
        std::cout << MC.bsp_vertices[MC.bsp_faces[i].vertices[2]] << std::endl;;

        std::cout << MC.m_vertices[MC.m_faces[*bsp_faces[i].div_faces.begin()][0]] << std::endl;
        std::cout << MC.m_vertices[MC.m_faces[*bsp_faces[i].div_faces.begin()][1]] << std::endl;
        std::cout << MC.m_vertices[MC.m_faces[*bsp_faces[i].div_faces.begin()][2]] << std::endl;;
        ...
        for (auto it = bsp_faces[i].div_faces.begin(); it != bsp_faces[i].div_faces.end(); it++) {
            std::array<Point_2, 3> df_vs; // target face 投影到2d
            for (int j = 0; j < 3; j++)
            ...
            for (int j = 0; j < 3; j++) {
                ...
                //对于target face中的一条边，如果bsp的face 3顶点不是位于该边的同侧  就把此边收入arr_segs
