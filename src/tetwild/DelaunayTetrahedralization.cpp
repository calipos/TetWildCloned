...
void DelaunayTetrahedralization::init(const std::vector<Point_3>& m_vertices, const std::vector<std::array<int, 3>>& m_faces,
    ...
    raw_e_tags = std::vector<int>(m_edges.size(), -1);//edge id
    raw_conn_e4v = std::vector<std::vector<int>>(m_vertices.size(), std::vector<int>());//一个vect对应哪些edge，有一个edge id
...
//    }
    ...
    //get bsp nodes
    int cnt = 0;
    std::vector<std::array<int, 4>> cells;// 每一个cell里面vertex的idx
    cells.reserve(T.number_of_finite_cells());
    std::vector<std::vector<int>> conn_n_ids(points.size(), std::vector<int>());//每一个vertex和哪些vertex idx相连
    ...
    std::vector<std::vector<int>> conn_f_ids(points.size(), std::vector<int>());//每一个vertex被哪些face 包含
