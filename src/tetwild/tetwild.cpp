...
int outBSPobj(
    const std::vector<Point_3>& bsp_vertices, 
    const std::vector<BSPEdge>& bsp_edges,
    const std::vector<BSPFace>& bsp_faces,
    const std::vector<BSPtreeNode>& bsp_nodes, 
    const std::string&path)
{
    std::fstream fout(path,std::ios::out);
    for (int i = 0; i < bsp_vertices.size(); i++)
    {
        fout << "v " << bsp_vertices[i].x() << " " << bsp_vertices[i].y() << " " << bsp_vertices[i].z() << std::endl;
    }
    for (int i = 0; i < bsp_faces.size(); i++)
    {
        fout << "f " << bsp_faces[i].vertices[0]+1 << " " << bsp_faces[i].vertices[1]+1 << " " << bsp_faces[i].vertices[2]+1 << std::endl;
    }
    fout.close();
    return 0;
}
    ...
    std::vector<std::vector<int>> &raw_conn_e4v)
{
    ...
    m_f_tags.clear();
    raw_e_tags.clear();//edge id
    raw_conn_e4v.clear();//一个vect对应哪些edge，有一个edge id
    ...
    std::vector<std::array<int, 4>> &is_surface_facet)
{
    ...
    std::vector<BSPFace> bsp_faces;
    std::vector<BSPtreeNode> bsp_nodes;
    std::vector<int> m_f_tags;//target face 的编号
    std::vector<int> raw_e_tags;// target edge 的编号
    std::vector<std::vector<int>> raw_conn_e4v;//一个vect对应哪些edge，有一个edge id
    ...
    outBSPobj(bsp_vertices, bsp_edges, bsp_faces, bsp_nodes,  state.working_dir + state.postfix + ".BSP.obj");
    //mesh conforming
    ...
    outBSPobj(bsp_vertices, bsp_edges, bsp_faces, bsp_nodes, state.working_dir + state.postfix + ".BSP.divided.obj");
