...
void MeshConformer::matchDivFaces() {
    std::vector<std::array<int, 2>> seed_v_list;//[0]存放bsp face中的vert id(在bsp vertix中，前面一部分是目标vertex，后面才是voxel的vertex)，[1]存放face的id 总长度是3*bsp_faces.size()
    ...
    for (int i = 0; i < m_faces_size; i++) {
        ...
        ///find seed info
        ///seed_fids :对于某一个target face， 把所有 和target face 的共享vert的bsp face包含进来 
        ///seed_nids :对于某一个target face，没有match到bsp的face，然后会把所有 和target face 的共享vert的bsp cell包含进来
        std::unordered_set<int> seed_fids, seed_nids;
        std::vector<std::vector<int>> f_lists;
        is_matched[i] = true;//按道理说  target的点应该都会被bsp中的点所match
        for (int j = 0; j < 3; j++) {
            std::vector<int> f_list;
            matchVertexIndices(m_faces[i][j], seed_v_list, f_list);//这一步在bsp中 把所有包含target pts的face id选出来了
            if (f_list.size() == 0)
                is_matched[i] = false;
        ...
        if (is_matched[i]) {
            ...
            if (tmp1.size() == 1) {
                ...
                if (tmp_bsp == tmp_m) {//三角剖分的面和target的面是一致的 match
                    is_matched[i] = true;
        ...
        while (true) {
            new_fids.clear();
            //这个循环只是在扫一遍 看有没有漏掉的 triangleIntersection3d
